...
from utils.dataset import WebVid10M, Trajectory_Data, Trajectory_blender_Data
...
def load_images_trajs_from_folder(folder, images_bbox=False):
    ...
    # print(image_files)
    traj_files = sorted(os.listdir(os.path.join(folder, "traj_vid_enhanced")))
    # print(traj_files)
    ...
    for filename in traj_files:
        ext = os.path.splitext(filename)[1].lower()
        if ext in valid_extensions:
            img = Image.open(os.path.join(folder, "traj_vid_enhanced", filename)).convert('RGB')
            trajs.append(img)
...
def make_train_traj_blender_dataset(args):
    # Get the datasets: you can either provide your own training and evaluation files (see below)
    # or specify a Dataset from the hub (the dataset will be downloaded automatically from the datasets Hub).

    # In distributed training, the load_dataset function guarantees that only one local process can concurrently
    # download the dataset.
    dataset = Trajectory_blender_Data(path=args.video_folder, sample_size=(args.height, args.width), images_bbox=args.images_bbox, filter_num=args.filter_num, cut_num=args.cut_num)
    return dataset
...
def main():
    ...
    train_dataset = make_train_traj_blender_dataset(args)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (args.validation_steps != 0 and
                        ...
                        validation_images, validation_control_images = load_images_trajs(args.validation_image_folder, args.images_bbox)
