...
def load_images_trajs_json(folders, size, vipseg_path):
    all_images = []
    all_trajs = []
...
def make_train_traj_VIPSeg_dataset(args):
    ...
    # In distributed training, the load_dataset function guarantees that only one local process can concurrently
    # download the dataset.
    dataset = Trajectory_VIPSeg_Data(path=args.video_folder, repeat_times=1, split_file="dataset/VIPSeg/train.txt", train_mode="split_vid", sample_size=(args.height, args.width))
    return dataset
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        validation_images, validation_control_images = load_images_trajs_json(args.validation_image_folder, (args.height, args.width), os.path.join(args.video_folder, "trajectory_CoTracker_gt"))
