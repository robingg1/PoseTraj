...
class StableVideoDiffusionPipelineControlNet(DiffusionPipeline):
    ...
    def __call__(
        ...
        camera_cond:[torch.FloatTensor] = None,
        height: int = 576,
        width: int = 1024,
        ...
        camera_cond = torch.tensor(camera_cond, dtype=torch.float32)
        camera_cond = camera_cond.unsqueeze(0)
        camera_cond = torch.cat([camera_cond] * 2) 
        camera_cond = camera_cond.to(device, latents.dtype)
