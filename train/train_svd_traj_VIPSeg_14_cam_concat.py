...
def make_train_traj_VIPSeg_dataset(args):
    ...
    # In distributed training, the load_dataset function guarantees that only one local process can concurrently
    # download the dataset.
    dataset = Trajectory_VIPSeg_Data(path=args.video_folder, repeat_times=1, split_file="/minimax-3d-rw/tuolaji/fyp/dataset/VIPSeg/train.txt", train_mode="split_vid", return_cam=True, camera_path=args.cam_path)
    return dataset
