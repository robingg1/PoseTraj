...
class ControlNetConditioningEmbeddingSVD(nn.Module):
    ...
    def __init__(
        ...
        self.rot_embed = nn.Embedding(2, conditioning_embedding_channels)

    def forward(self, conditioning, rot):
        ...
        print(rot)
        rot_embedding = self.rot_embed(rot).repeat_interleave(batch_size * frames, dim=0).view(batch_size * frames, embedding.shape[1], 1, 1).expand(-1, -1, embedding.shape[2], embedding.shape[3])
        embedding += rot_embedding
...
class ControlNetSDVModel(ModelMixin, ConfigMixin, FromOriginalControlnetMixin):
    ...
    def forward(
        ...
        controlnet_rot_id: torch.FloatTensor = None,
        ...
        if controlnet_cond != None:
            controlnet_cond = self.controlnet_cond_embedding(controlnet_cond, controlnet_rot_id)
