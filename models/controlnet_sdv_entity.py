...
class ControlNetConditioningEmbeddingSVD(nn.Module):
    ...
    def forward(self, conditioning, feature_entity):
        ...
        conditioning_2 = feature_entity.repeat(batch_size * frames, 1, 1, 1)
        embedding_2 = self.conv_in_2(conditioning_2)
        embedding_2 = self.conv_out_2(embedding_2)

        embedding = embedding + embedding_2
...
class ControlNetSDVModel(ModelMixin, ConfigMixin, FromOriginalControlnetMixin):
    ...
    def forward(
        ...
        controlnet_bbox: torch.FloatTensor = None,
        ...
        if controlnet_cond != None:
            controlnet_cond = self.controlnet_cond_embedding(controlnet_cond, controlnet_bbox)
